...
for mbs in mini_batch_sizes:
    for nunits in no_units:
        mynn = keras.Sequential([
            keras.layers.Dense(units=nunits, activation='relu', input_dim=no_inputs),  # Hidden layer with c[1] units
            keras.layers.Dense(units=1, activation='sigmoid')  # output layer with one unit
        ])
    
        mynn.compile(optimizer='Adam', loss='binary_crossentropy', metrics=['binary_accuracy'])
        mynn.fit(x=X_train, y=y_train, batch_size=mbs, epochs=5, validation_data=(X_val, y_val))
    
        _, binary_accuracy = mynn.evaluate(x=X_test, y=y_test)
        results[(mbs, nunits)] = binary_accuracy
