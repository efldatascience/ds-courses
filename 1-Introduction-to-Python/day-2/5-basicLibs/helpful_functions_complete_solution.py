...
print(files)
all_customer_matrix = np.empty(shape=(0, 3))  # store data of each customer

for file in files:
    filepath = os.path.join(path, file)
    with open(filepath, "r") as customerfile:
        reader = csv.reader(customerfile, delimiter=",")
        output = []
        for i, row in enumerate(reader):
            if i > 0:  # skip first row of csv file
                input_list = list(map(int, row[1:]))  # skip first value of row and convert to integers
                output.append(input_list)
    customer_matrix = np.array(output)

    # Print average consumption for customer i
    print(f"Mean consumption of {file.split('.')[0]}")
    print("Apples, Bananas, Citrons")
    print(customer_matrix.mean(axis=0))

    print(f"Standard deviation of {file.split('.')[0]}")
    print("Apples, Bananas, Citrons")
    print(customer_matrix.std(axis=0))

    # Append data to full array
    all_customer_matrix = np.append(all_customer_matrix, customer_matrix, axis=0)

print(f"Mean consumption across all customers")
print("Apples, Bananas, Citrons")
print(all_customer_matrix.mean(axis=0))

print(f"Standard deviation across all customers")
print("Apples, Bananas, Citrons")
print(all_customer_matrix.std(axis=0))
